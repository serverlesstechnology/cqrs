...
/// In CQRS (and Domain Driven Design) an `Aggregate` is the fundamental
/// component that encapsulates the state and application logic (aka business
/// rules) for the application. An `Aggregate` is always composed of a
...
pub trait Aggregate: Default + Serialize + DeserializeOwned + Sync + Send {
    /// Specifies the inbound command used to make changes in the state of the
    /// Aggregate.
    type Command;
    /// Specifies the published events representing some change in state of the
    /// Aggregate.
    type Event: DomainEvent;
    /// The error returned when a command fails due to business logic.
    /// This is used to provide feedback to the user as to the nature of why the
    /// command was refused.
    type Error: std::error::Error;
    /// The external services required for the logic within the Aggregate
    type Services: Send + Sync;
    /// The aggregate type is used as the unique identifier for this aggregate
    /// and its events. This is used for persisting the events and snapshots
    /// to a database.
    fn aggregate_type() -> String;
    /// This method consumes and processes commands.
    /// The result should be either a vector of events if the command is
    /// successful, or an error if the command is rejected.
    ///
    ...
    /// This is used to update the aggregate's state once an event has been
    /// committed. Any events returned from the `handle` method will be
    /// applied using this method in order to populate the state of the
    /// aggregate instance.
    ///
    /// The source of truth used in the CQRS framework determines when the
    /// events are applied to an aggregate:
    /// - event sourced - All events are applied every time the aggregate is
    ///   loaded.
    /// - aggregate sourced - Events are applied immediately after they are
    ///   returned from `handle`
    /// (and before they are committed) and the resulting aggregate instance is
    /// serialized and persisted.
    /// - snapshots - Uses a combination of the above patterns.
    ///
    /// _No business logic should be placed here_, this is only used for
    /// updating the aggregate state.
    ///
    /// ```rust
    /// # use std::sync::Arc;
    /// # use serde::{Serialize, Deserialize, de::DeserializeOwned};
