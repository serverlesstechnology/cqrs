...
impl Command<TestAggregate, TestEvent> for CreateTest {
    fn handle(&self, _aggregate: &mut TestAggregate) -> Result<Vec<TestEvent>, AggregateError> {
...
impl<'a> Command<TestAggregate, TestEvent> for ConfirmTest<'a> {
    fn handle(&self, aggregate: &mut TestAggregate) -> Result<Vec<TestEvent>, AggregateError> {
